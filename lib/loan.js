 ...
 * @param {String}  [props.pay_every="week"] - Period of payment: week/month/year/quarter
 ...
 * @param {Date}    [props.first_payment_date] - Data current as of
 * @param {Object}  [props.data] - Extra data
 * @param {Number}  [props.balloon_at=0] - Balloon Item Number to close
 * @param {Boolean} [props.is_balloon_payment=false]  
 * @param {Number}  [props.to_pay_to_close] - Amount due current period
...
Loan.prototype.getPaymentPlan = function() {
  ...
  switch(type) {
    ...
    case 'annuity':
      ...
      return _.reduce(range.slice(0,(self.balloon_at > 0 ? (self.balloon_at) : self.instalments)), function(plan, instalment) {
        if(self.balloon_at > 0 && instalment == (self.balloon_at - 1))
        {
          
          console.log('sum:', _.sumBy(range.slice((self.balloon_at - 1), self.instalments - 1), 'to_pay'))
          var isFirst = plan.length <= 0;
          var payment = isFirst ? self.toObject() : _.clone(_.last(plan));
          ...
          payment.as_of = self.getDateOfInstalment(instalment);
          payment.is_balloon_payment = true;
          payment.to_pay_to_close = payment.principal + payment.interest
        ...
        else
        {
          ...
          payment.to_pay = toPay;
          payment.as_of = self.getDateOfInstalment(instalment);
          payment.balloon_at = self.balloon_at;
          payment.is_balloon_payment = false;
...
Loan.prototype.getInterestRatePerInstalment = function() {
  ...
  switch(this.pay_every) {
    ...
    case 'week': return this.interest_rate / 52;
    case 'biweek': return this.interest_rate / 26;
...
Loan.prototype.getDateOfInstalment = function(instalment) {
  ...
  if(this.pay_every == 'biweek')
  {
    return moment(this.getAsOf()).add(instalment * 14, 'days').toDate();
  }
  return moment(this.getAsOf()).add(instalment, this.pay_every).toDate();
};
...
Loan.prototype.shouldAmortize = function(compare) {
  ...
  return asOf.toDate() < compare.toDate();
};
